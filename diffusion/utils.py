...
def wavelet_decompose_batch(x, wavelet='db4'):
    """
    Perform single-level DWT on a batch of signals.
    Args:
        x: Tensor of shape (batch, channels, length)
        wavelet: Type of wavelet to use
    Returns:
        cA: Approximation coefficients (batch, channels, length//2)
        cD: Detail coefficients (batch, channels, length//2)
    """
    batch, channels, length = x.shape
    cA_list, cD_list = [], []

    for i in range(batch):
        cA_channels, cD_channels = [], []
        for j in range(channels):
            signal = x[i, j].cpu().numpy()
            cA, cD = pywt.dwt(signal, wavelet, mode='periodization')
            cA_channels.append(torch.tensor(cA))
            cD_channels.append(torch.tensor(cD))
        cA_list.append(torch.stack(cA_channels))  # (channels, new_length)
        cD_list.append(torch.stack(cD_channels))
