...
from diffusion.diffusion_model import DiffusionAttnUnet1DCond, DiffusionAttnDualBranchUnet1D, DiffusionAttnUnet1D
...
class Trainer:
    ...
    def train_one_epoch_conditional(self,epoch):
        ...
        for idx,batch in enumerate(self.train_loader):
            ...
            if torch.cuda.device_count() < 2 :                                                       
                loss = self.model.training_step(y,fc,self.lambda_corr,cond = cond_low, device = self.device)
            else:
                loss = self.model.module.training_step(y,fc,self.lambda_corr, device = self.device)
        ...
        with torch.no_grad():
            ...
            for idx,batch in enumerate(self.test_loader):
                ...
                if torch.cuda.device_count() < 2 :                                                       
                    batch_dict = self.model.test_step(y,x = cond_low, num_steps= self.test_step, device = self.device)
                else:
                    batch_dict = self.model.module.test_step(y,x = cond_low, num_steps= self.test_step, device = self.device)
...
class TrainerDualBranch:
    ...
    def save_model(self, epoch, filename="training_1e-3_20_epochs.pth"):
        if self.accelerate.is_local_main_process:
            try:
                ...
                run_name = ""
            model = self.accelerate.unwrap_model(self.model)
            folder_path = os.path.join(self.saving_path, self.diffusion_mode, run_name)
