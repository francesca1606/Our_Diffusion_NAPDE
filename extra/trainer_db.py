...
from extra.db_model import DiffusionAttnDualBranchUnet1D
            ...
            for idx, batch in enumerate(self.test_loader):
                ...
                if torch.cuda.device_count() < 1:
                    batch_dict = self.model.test_step(
                        y, x, num_steps=self.test_step, device=self.device)
                else:
    ...
    def save_model(self, epoch, filename="training_1e-3_20_epochs.pth"):
        if self.accelerate.is_local_main_process:
            ...
            folder_path = os.path.join(
                self.saving_path, self.diffusion_mode, run_name)
